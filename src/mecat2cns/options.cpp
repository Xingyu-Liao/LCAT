...
static double identity_threshold_pacbio    = 0.65;
static int slide_window_length_pacbio      = 75;
static double min_modify_coverage_pacbio   = 0.05;
...
static double identity_threshold_nanopore    = 0.65;
static int slide_window_length_nanopore      = 75;
static double min_modify_coverage_nanopore   = 0.05;
...
static const char identity_threshold_n  = 'd';
static const char window_length_n = 'w';
static const char min_modify_coverage_n = 'm';

void
print_pacbio_default_options()
{
	cerr << '-' << num_threads_n << ' ' << num_threads_pacbio
		 << ' '
		 ...
		 << ' '
		 << '-' << identity_threshold_n << ' ' << identity_threshold_pacbio 
		 << ' '
		 << '-' << window_length_n << ' ' << slide_window_length_pacbio
		 << ' '
		 << '-' << min_modify_coverage_n << ' ' << min_modify_coverage_pacbio
		 << "\n";
}

void print_nanopore_default_options()
{
	cerr << '-' << num_threads_n << ' ' << num_threads_nanopore
		 << ' '
		 ...
		 << '-' << num_partition_files_n << ' ' << num_partition_files
		 << ' '
		 << '-' << identity_threshold_n << ' ' << identity_threshold_nanopore
		 << ' '
		 << '-' << window_length_n << ' ' << slide_window_length_nanopore
		 << ' '
		 << '-' << min_modify_coverage_n << ' ' << min_modify_coverage_nanopore
		 << "\n";
}
...
print_usage(const char* prog)
{
	...
	cerr << "-" << identity_threshold_n << " <Real>\t" 
		 << "identity threshold" 
		 << "\n";
	cerr << "-" << window_length_n << " <Integer>\t" 
		 << "slide window length" 
		 << "\n";
	cerr << "-" << min_modify_coverage_n << " <Real>\t" 
		 << "minimum coverage rate of modify region" 
		 << "\n";
...
init_consensus_options(int tech)
{
	...
	if (tech == TECH_PACBIO) {
		...
		t.tech                  = tech_pacbio;
		t.identity_threshold    = identity_threshold_pacbio;
		t.window_length         = slide_window_length_pacbio;
		t.min_modify_coverage   = min_modify_coverage_pacbio;
	} else {
		...
		t.identity_threshold    = identity_threshold_nanopore;
		t.window_length         = slide_window_length_nanopore;
		t.min_modify_coverage   = min_modify_coverage_nanopore;
	}
    return t;
}
...
parse_arguments(int argc, char* argv[], ConsensusOptions& t)
{
    ...
    char err_char;
    opterr = 0;
    while((opt_char = getopt(argc, argv, "t:p:r:a:c:l:x:k:h:d:w:m")) != -1) {
	//while((opt_char = getopt(argc, argv, "i:t:p:r:a:c:l:x:k:h:iden:winl:mc")) != -1) {
		switch (opt_char) {
			/*
			case input_type_n://i
				if (optarg[0] == '0')
			...
			case tech_n://x
				break;
			case num_partition_files_n://k
				t.num_partition_files = atoi(optarg);
				break;
			case identity_threshold_n://iden/d
				t.identity_threshold = atof(optarg);
				break;
			case window_length_n://winl/w
				t.window_length = atoi(optarg);
				break;
			case min_modify_coverage_n://mc/m
				t.min_modify_coverage = atof(optarg);
				break;
			case '?':
	...
	if (t.identity_threshold < 0.0 || t.identity_threshold >=1.0)
	{
		std::cerr << "identity threshold must between [0-1)\n";
		parse_success = false;
	}
	if (t.window_length <= 0)
	{
		std::cerr << "slide window length must be greater than 0\n";
		parse_success = false;
	}
	if (t.min_modify_coverage < 0.0)
	{
		std::cerr << "minimum modify coverage rate must be >= 0.0\n";
		parse_success = false;
	}
...
print_options(ConsensusOptions& t)
{
	...
	cout << "tech:\t" << t.tech << "\n";
	cout << "identity threshold:\t" << t.identity_threshold << "\n";
	cout << "slide window length:\t" << t.window_length << "\n";
	cout << "minimum modify coverage rate:\t" << t.min_modify_coverage << "\n";
}
