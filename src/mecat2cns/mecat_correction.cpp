...
identify_one_consensus_item(CnsTableItem& cns_item, CnsTableItem& cns_item_new, const int min_cov, const double min_modify_coverage)
{
	...
	if (cns_item_new.skip_cnt >= cov_all * min_modify_coverage){
				   ...
				   std::string& aux_tstr, std::string& cns)
{
	...
	ag.mergeNodes();
	ag.consensus(temp_min_cov * 0.4, cns);
}
void
						   ...
						   std::string& target, const int min_cov, const double min_modify_coverage)
{
	for (int i = 0; i < cns_list_size; ++i) {
		cns_id_vec[i] = identify_one_consensus_item(cns_list[i], cns_list_new[i], min_cov, min_modify_coverage);
	}
	int i = 0, j; 
	std::string cns;
	target.clear();
	while (i < cns_list_size && (!(cns_id_vec[i] & FMAT) && !(cns_id_vec[i] & M_FMAT))) ++i;
	while (i < cns_list_size)
	{
		target.push_back(cns_list[i].base);
		j = i + 1;
		while (j < cns_list_size && (!(cns_id_vec[j] & FMAT) && !(cns_id_vec[j] & M_FMAT))) ++j;
...
consensus_one_read_can_pacbio(ConsensusThreadData* ctd, const index_t read_id, const index_t sid, const index_t eid)
{
	...
	M5Record* m5 = ctd->m5;
	CnsAlns& cns_vec = ctd->cns_alns;
	CnsAlns& cns_vec_new = ctd->new_cns_alns;
	std::vector<CnsResult>& cns_results = ctd->cns_results;
	...
	CnsTableItem* cns_table = ctd->cns_table;
	CnsTableItem cns_table1[read_size];
	memcpy(cns_table1, cns_table, read_size);
	CnsTableItem* cns_table_new = cns_table1;
	std::for_each(cns_table_new, cns_table_new + read_size, CnsTableItemCleaner());
	std::for_each(cns_table, cns_table + read_size, CnsTableItemCleaner());
	cns_vec.clear();
	cns_vec_new.clear();
